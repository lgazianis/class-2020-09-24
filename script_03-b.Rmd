...
```{r}
sep_full <- full_join(sep_old, sep_new, by = c("title", "id", "department"))
```
...
library(gt)
x <- sep_full %>%
  mutate(ugrad_diff = u_grad.y - u_grad.x) %>%
  select(id, title, name.x, ugrad_diff) %>%
